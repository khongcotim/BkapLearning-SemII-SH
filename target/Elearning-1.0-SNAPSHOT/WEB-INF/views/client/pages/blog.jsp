<%--
  Created by IntelliJ IDEA.
  User: bt289
  Date: 3/30/2022
  Time: 11:45 AM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<html>
<head>
  <title>Elearning | Blog</title>
  <link rel="shortcut icon" href="<c:url value="/public/assets/icon/favicon.png"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/animate.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/bootstrap.min.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/font-awesome.min.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/icofont.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/slick.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/magnific-popup.css"/>">
  <link rel="stylesheet" href="<c:url value="/public/assets/css/style.css"/>">
</head>
<body>
<c:url value="/" var="rootpath"></c:url>
<jsp:include page="/WEB-INF/views/client/menu/main_menu.jsp"></jsp:include>
<jsp:include page="/WEB-INF/views/client/header/header.jsp"></jsp:include>

<!-- Breadcrumb Section Starts -->
<section class="breadcrumb-section">
  <div class="breadcrumb-shape">
    <img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="shape" class="hero-round-shape-2 item-moveTwo">
    <img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="shape" class="hero-plus-sign item-rotate">
  </div>
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
        <h2>blog page</h2>
        <div class="breadcrumb-link margin-top-10">
          <span><a href="index.html">home</a> / blog page</span>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- Blog Content Section -->
<section class="blog-content-section padding-120">
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
        <div class="blog-posts">
          <div class="blog-single-post">
            <div class="blog-thumbnail">
              <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="thumbnail"></a>
            </div>
            <div class="blog-content-part">
              <div class="blog-content-top">
                <div class="blog-date margin-right-20">
                  <a href="#"><i class="fa fa-calendar"></i>28 june 2020</a>
                </div>
                <div class="blog-tag margin-right-20">
                  <a href="#"><i class="fa fa-tag"></i>web design</a>
                </div>
                <div class="blog-author">
                  <a href="#"><i class="fa fa-user-circle"></i>john doe</a>
                </div>
              </div>
              <div class="blog-title">
                <h3><a href="blog-details.html">they're getting to know learning app</a></h3>
              </div>
              <div class="blog-content">
                <p>Organizations today are in constant flux. Industries are consolidating, business models are emerging, new technologies are being developed, and consumer behaviors are evolving.</p>
              </div>
              <div class="blog-excerpt-link">
                <a href="blog-details.html">read more</a>
              </div>
            </div>
          </div>
          <div class="blog-single-post different-bg-color">
            <div class="blog-thumbnail-slider">
              <div class="thumbnail-single-slide">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="thumbnail"></a>
              </div>
              <div class="thumbnail-single-slide">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="thumbnail"></a>
              </div>
              <div class="thumbnail-single-slide">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="thumbnail"></a>
              </div>
            </div>
            <div class="blog-content-part">
              <div class="blog-content-top">
                <div class="blog-date margin-right-20">
                  <a href="#"><i class="fa fa-calendar"></i>28 june 2020</a>
                </div>
                <div class="blog-tag margin-right-20">
                  <a href="#"><i class="fa fa-tag"></i>web design</a>
                </div>
                <div class="blog-author">
                  <a href="#"><i class="fa fa-user-circle"></i>john doe</a>
                </div>
              </div>
              <div class="blog-title">
                <h3><a href="blog-details.html">they're getting to know learning app</a></h3>
              </div>
              <div class="blog-content">
                <p>Organizations today are in constant flux. Industries are consolidating, business models are emerging, new technologies are being developed, and consumer behaviors are evolving.</p>
              </div>
              <div class="blog-excerpt-link">
                <a href="blog-details.html">read more</a>
              </div>
            </div>
          </div>
          <div class="blog-single-post">
            <div class="blog-thumbnail-video">
              <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="thumbnail"></a>
              <div class="video-play-button">
                <a href="https://www.youtube.com/watch?v=8AGgbIQyqR8" class="button-video">
                  <i class="fa fa-play item-ripple"></i>
                </a>
              </div>
            </div>
            <div class="blog-content-part">
              <div class="blog-content-top">
                <div class="blog-date margin-right-20">
                  <a href="#"><i class="fa fa-calendar"></i>28 june 2020</a>
                </div>
                <div class="blog-tag margin-right-20">
                  <a href="#"><i class="fa fa-tag"></i>web design</a>
                </div>
                <div class="blog-author">
                  <a href="#"><i class="fa fa-user-circle"></i>john doe</a>
                </div>
              </div>
              <div class="blog-title">
                <h3><a href="blog-details.html">they're getting to know learning app</a></h3>
              </div>
              <div class="blog-content">
                <p>Organizations today are in constant flux. Industries are consolidating, business models are emerging, new technologies are being developed, and consumer behaviors are evolving.</p>
              </div>
              <div class="blog-excerpt-link">
                <a href="blog-details.html">read more</a>
              </div>
            </div>
          </div>
          <div class="blog-single-post different-bg-color">
            <div class="blog-content-part">
              <div class="blog-content-top">
                <div class="blog-date margin-right-20">
                  <a href="#"><i class="fa fa-calendar"></i>28 june 2020</a>
                </div>
                <div class="blog-tag margin-right-20">
                  <a href="#"><i class="fa fa-tag"></i>web design</a>
                </div>
                <div class="blog-author">
                  <a href="#"><i class="fa fa-user-circle"></i>john doe</a>
                </div>
              </div>
              <div class="blog-title">
                <h3><a href="blog-details.html">they're getting to know learning app</a></h3>
              </div>
              <div class="blog-content">
                <p>Organizations today are in constant flux. Industries are consolidating, business models are emerging, new technologies are being developed, and consumer behaviors are evolving.</p>
              </div>
              <div class="blog-excerpt-link">
                <a href="blog-details.html">read more</a>
              </div>
            </div>
          </div>
        </div>
        <div class="template-pagination">
          <ul>
            <li class="pagination-arrow"><a href="#"><i class="fa fa-angle-left"></i></a></li>
            <li><a href="#">01</a></li>
            <li><a href="#">02</a></li>
            <li><a href="#">03</a></li>
            <li><a href="#">04</a></li>
            <li class="pagination-arrow"><a href="#"><i class="fa fa-angle-right"></i></a></li>
          </ul>
        </div>
      </div>
      <div class="col-lg-4">
        <div class="blog-sidebar">
          <div class="single-widget author-widget">
            <img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image">
            <div class="author-name margin-top-20">
              <h4>john doe</h4>
            </div>
            <div class="author-content margin-top-10">
              Hi! I'm author of this post. Read my post, be in trend.
            </div>
            <div class="author-social-link margin-top-20">
              <ul>
                <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                <li><a href="#"><i class="fa fa-instagram"></i></a></li>
              </ul>
            </div>
          </div>
          <div class="single-widget search-widget">
            <div class="header-search">
              <form action="#">
                <input type="text" placeholder="Search">
                <button type="submit"><i class="fa fa-search"></i></button>
              </form>
            </div>
          </div>
          <div class="single-widget recent-post-widget">
            <div class="widget-title">
              <h4>recent posts</h4>
            </div>
            <div class="single-recent-post">
              <div class="recent-post-image">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image"></a>
              </div>
              <div class="recent-post-title">
                <div class="post-date">
                  <a href="#"><i class="fa fa-calendar"></i>20 June 2020</a>
                </div>
                <h5><a href="blog-details.html">twice profit than before you ever got business</a></h5>
              </div>
            </div>
            <div class="single-recent-post">
              <div class="recent-post-image">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image"></a>
              </div>
              <div class="recent-post-title">
                <div class="post-date">
                  <a href="#"><i class="fa fa-calendar"></i>28 June 2020</a>
                </div>
                <h5><a href="blog-details.html">twice profit than before you ever got business</a></h5>
              </div>
            </div>
            <div class="single-recent-post">
              <div class="recent-post-image">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image"></a>
              </div>
              <div class="recent-post-title">
                <div class="post-date">
                  <a href="#"><i class="fa fa-calendar"></i>20 September 2020</a>
                </div>
                <h5><a href="blog-details.html">twice profit than before you ever got business</a></h5>
              </div>
            </div>
            <div class="single-recent-post">
              <div class="recent-post-image">
                <a href="blog-details.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image"></a>
              </div>
              <div class="recent-post-title">
                <div class="post-date">
                  <a href="#"><i class="fa fa-calendar"></i>14 October 2020</a>
                </div>
                <h5><a href="blog-details.html">twice profit than before you ever got business</a></h5>
              </div>
            </div>
          </div>
          <div class="single-widget category-widget">
            <div class="widget-title">
              <h4>all categories</h4>
            </div>
            <div class="category-items">
              <ul>
                <li><a href="#" class="border-none"><i class="fa fa-circle"></i>web development (03)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>education (02)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>business (05)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>finance and banking (03)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>marketing (02)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>photography (08)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>technology (09)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>UI & UX design (05)</a></li>
                <li><a href="#"><i class="fa fa-circle"></i>psychology (02)</a></li>
              </ul>
            </div>
          </div>
          <div class="single-widget tag-widget">
            <div class="widget-title">
              <h4>tags</h4>
            </div>
            <div class="tag-items">
              <ul>
                <li><a href="#">app</a></li>
                <li><a href="#">agency</a></li>
                <li><a href="#">history</a></li>
                <li><a href="#">education</a></li>
                <li><a href="#">learning</a></li>
                <li><a href="#">web design</a></li>
                <li><a href="#">ux design</a></li>
                <li><a href="#">optimize</a></li>
                <li><a href="#">online</a></li>
                <li><a href="#">course</a></li>
              </ul>
            </div>
          </div>
          <div class="single-widget banner-widget">
            <div class="banner-widget-logo">
              <a href="index.html"><img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="logo"></a>
            </div>
            <div class="banner-widget-title text-center margin-top-20">
              <h4>start course in yourself today</h4>
            </div>
            <div class="banner-widget-button text-center margin-top-30">
              <a href="#" class="template-button">instructor</a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- CTA Section Starts -->
<section class="cta-section gradient-bg padding-top-60 padding-bottom-30">
  <div class="cta-shape">
    <img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image" class="plus-sign item-rotate">
  </div>
  <div class="container">
    <div class="row align-items-center">
      <div class="col-lg-6">
        <div class="section-title margin-bottom-40">
          <h2>enhance your skills with <span>best online course</span></h2>
        </div>
        <div class="cta-button">
          <a href="#" class="template-button margin-right-20">start teaching</a>
          <a href="#" class="template-button-2">start learning</a>
        </div>
      </div>
      <div class="col-xl-4 offset-xl-2 col-lg-6">
        <div class="cta-image">
          <img src="<c:url value="/public/assets/images/purchase-image-1.jpg"/>" alt="image">
        </div>
      </div>
    </div>
  </div>
</section>


<jsp:include page="/WEB-INF/views/client/footer/footer.jsp"></jsp:include>
</body>
</html>
